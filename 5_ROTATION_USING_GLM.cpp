...
//refer https://math.stackexchange.com/questions/3279863/2d-transformation-matrix-order
...
static const char* vShader = R"gl(
  ...
  void main()
  {
    gl_Position = model* vec4(0.4*pos.x,0.4*pos.y,0.4*pos.z,1);
  }
)gl";
...
int main()
{
    ...
    while (!glfwWindowShouldClose(MainWindow))
    {
        ...
        glm::mat4 model(1.f);
        //Multiplying Identity matrix with transforms to get transformation matrix
        //Stuff happens in the reverse order of how to code it
        //Eg. calling translate() first and then rotate(),it will seemingly first rotate the object and then translate it along x and y axis
        //Conversly calling rotate() first and then translate() will translate it first along x and y and then rotate the translated matrix causing it to skew
        //There will be some distortion in the triangle
        //This is because the triangle is being rendered relative to the screen and not in it's own coordinate system
